...
`jcv-maven-plugin`是一个自动为网页添加js css的版本号maven插件,并且支持js css的自动压缩，支持多种方法版本号添加，该插件自动采集文件的md5值进行文件版本号修订，这个可以在每次更新的时候只修改以修改文件的版本号，防止全部js css缓存失效，目前该插件以发布到mvn中央仓库,可以坐标引用使用.
